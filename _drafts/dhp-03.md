...
> + 注意此處的 `uju`，在 __108__ 中為 _ujju-_。
> + Rau 指出 `ujuṃ karoti` 等同於 __80 145__ 中的 _namayanti_。
...
> + 注釋以為第一個 `oka` 意為水。_oka_ 表示家，參見 __87 91__。`oka-m-okata` 中的連聲，參見 __103 247 390__。
> + __Dhp-a__: `ubbhato` _ti uddhato_. __PED__ 以為來自 Skt _uddhṛta_，但 __WD__ 以為沒有理由懷疑來自 Skt _udbhṛta_。
> + 注釋，或它所承接的傳統似乎不認為 `pahātave` 是表目的的不定式，而將 _-tave_ 看作 _-tavve_，即將來被動分詞中性單數的東方形態。東方形態參見 __32__ 注。不定式語尾參見 _netave_ __180__。
...
> + 注釋以為 `mokkhanti` 是將來被動形式，參見 _pamokkhanti_ __276__。
...
> + 關於 `an-avassuta`，參見 _an-ussuta_ __400__。
...
> + __Dhp-a__: `anivesano siyā` _ti anālayo bhaveyya_。
> + `āvudha`（Skt _āyudha_）中的 _v/y_ 變化是東方的一種特徵，參見 _māluvā_ __162__ 和 _parissaya_ __328__ 注。
...
> + __Udāna-v__ 改作 _na dveṣī dveṣiṇaḥ kuryād vairī vā vairiṇo hitam_。
> + 關於 `vā pana`，見 _vā puna_ __271__。
...
> + __Udāna-v__ 作 _na taṃ mātā pitā vāpi kuryāj ... cittaṃ yat kuryād dhitam ātmanaḥ_。
> + 關於 `seyyaso`（Skt _śreyas_），即在比較級 _seyya_ 上加後綴 _-so_，見 Geiger §100.3。
